...
class TrRun1(tornado.web.RequestHandler):
    '''
    使用 tr 的 run 方法
    '''
    ...
    def post(self):
        ...
        img_up = self.request.files.get('file', None)
        img_b64 = self.get_argument('img', None)
        img_network = self.get_argument('img_network', None)
        compress_size = self.get_argument('compress', None)
        ...
        if img_up is not None and len(img_up) > 0:
            ...
            raw_image = base64.b64decode(img_b64.encode('utf8'))
            img = Image.open(BytesIO(raw_image))
        elif img_network is not None:
            response = requests.get(img_network)
            if response.status_code == 200:
                ...
                logger.error(json.dumps({'code': 400, 'msg': '网络图片错误'}, cls=NpEncoder))
                self.finish(json.dumps({'code': 400, 'msg': '网络图片错误'}, cls=NpEncoder))
                return
        else:
            self.set_status(400)
            logger.error(json.dumps({'code': 400, 'msg': '没有传入参数'}, cls=NpEncoder))
